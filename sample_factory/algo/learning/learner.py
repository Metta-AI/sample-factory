...
from sample_factory.algo.utils.tensor_dict import shallow_recursive_copy
...
class Learner(Configurable):
    ...
    def _calculate_losses(
        self, mb: TensorDict, num_invalids: int
        ...
        with self.timing.add_time("forward_head"):
            self.actor_critic.forward_head(mb)
            minibatch_size: int = mb.batch_size
        ...
        with self.timing.add_time("tail"):
            ...
            self.actor_critic.forward_tail(mb, values_only=False, sample_actions=False)
            if self.cfg.aux_loss_coeff > 0.0:
                aux_loss = self.cfg.aux_loss_coeff * self.actor_critic.aux_loss(mb)
    ...
    def _prepare_batch(self, batch: TensorDict) -> Tuple[TensorDict, int, int]:
        with torch.no_grad():
            ...
            buff = TensorDict(shallow_recursive_copy(batch), batch_size=batch["actions"].shape[0])
            ...
            obs = buff["obs"].auto_batch_size_(batch_dims=2).view(-1)
            with self.param_server.policy_lock:
                normalized_obs = prepare_and_normalize_obs(self.actor_critic, obs)

            buff["normalized_obs"] = normalized_obs.view(*buff.batch_size, -1)
            del buff["obs"]  # don't need non-normalized obs anymore

            # calculate estimated value for the next step (T+1)
            next_state = TensorDict({
                "normalized_obs": buff["normalized_obs"][:, -1],
                "rnn_states": buff["rnn_states"][:, -1],
            })
            self.actor_critic(next_state, values_only=True)
            buff["values"][:, -1] = next_state["values"]
